///
<reference path="/ISPReferences/CV.en.isp.js" />
/*<!--*/
$subPage = 'CV';
$load('master/en.master.js')({
    title: 'CV',
    langs: langs,
    head:function(){
    
    /*--><script type="text/javascript">$('[title]').tooltip({ placement: 'bottom' });</script><!--*/
    },
    body: function () {

        /*-->
<div class="well visible-desktop">
    <h2 style="text-align:right;">Download this CV as 
        <a href="CV.en.pdf" class="btn btn-large">
            <i class=" icon-download-alt"></i>PDF format</a>
        <a href="CV.en.jpg" class="btn btn-large">
            <i class=" icon-download-alt"></i>JPG format</a></h2>
</div>
<div class="well">
    <h1>
        <span style="float: right">Web Front-end Developer</span>.NET Developer</h1>
</div>
<div class="row">
    <div class="span7">
        <h6>
            Curriculum Vitae</h6>
        <h2>
            C.V.</h2>
        <h3>
            Professional Skills</h3>
        <ul>
            <li>Familiar with developing on .NET platforms</li>
            <li>Programming using JavaScript/C#/C/C++</li>
            <li>Familiar with web-standard and popular front-end technologies</li>
            <li>Good object-oriented designing skills</li>
            <li>Good understanding about TCP/IP and HTTP protocol</li>
            <li>Likes mashupping!</li>
        </ul>
    </div>
    <div class="span5">
        <dl class="dl-horizontal well">
            <dt>Name:</dt>
            <dd>
                Zhixiang Yin ( Michael Yin )</dd>
            <dt>Age:</dt>
            <dd>
                22</dd>
            <dt>Page:</dt>
            <dd>
                <a href="http://zhixiang.in/" target="_blank">http://zhixiang.in/</a></dd>
            <dt>E-mail:</dt>
            <dd>
                layerssss<!---->(at)gmail.com</dd>
            <dt>Education:</dt>
            <dd>
                BS in Jinan University</dd>
            <dt></dt>
            <dd>
                ( will graduate at 2012 )</dd>
            <dt>Major:</dt>
            <dd>
                Softwere Engineering</dd>
            <dt>Languages:</dt>
            <dd>
                Mandarin Chinese, English, Esperanto, Italian</dd>
        </dl>
    </div>
    <div class="span12">
        <h3>
            Works:</h3>
        <div class="row-fluid">
            <div class="span3">
                &nbsp;</div>
            <div class="span4">
                Scope</div>
            <div class="span2">
                Start time (year/month)</div>
            <div class="span2">
                &nbsp;</div>
        </div>
        <div class="work">
            <div class="row-fluid">
                <div class="span3">
                    Gebimai.com</div>
                <div class="span4">
                    Server-side, weibo robot
                </div>
                <div class="span2">
                    2012/03</div>
                <div class="span3">
                    <a href="http://gebimai.com/" target="_blank">http://gebimai.com/</a>
                </div>
            </div>
        </div>
        <div class="work">
            <div class="row-fluid">
                <div class="span3">
                    Movieman's Website</div>
                <div class="span4">
                    Full development
                </div>
                <div class="span2">
                    2012/02</div>
                <div class="span3">
                    <a href="http://jacksonmovieman.com/" target="_blank">http://jacksonmovieman.com/</a>
                </div>
            </div>
        </div>
        <div class="work">
            <div class="row-fluid">
                <div class="span3">
                    ISP.js</div>
                <div class="span4">
                    Full development until now
                </div>
                <div class="span2">
                    2012/01</div>
                <div class="span3">
                    <a href="http://ispjs.org/" target="_blank">http://ispjs.org/</a>
                    <a href="https://github.com/layerssss/ispJs" target="_blank">source</a>
                </div>
            </div>
        </div>
        <div class="work">
            <div class="row-fluid">
                <div class="span3">
                    Goclassing.com</div>
                <div class="span4">
                    Full development until now
                </div>
                <div class="span2">
                    2011/12</div>
                <div class="span3">
                    <a href="http://goclassing.com/" target="_blank">http://goclassing.com/</a>
                    <a href="https://github.com/layerssss/goclassing.com" target="_blank">source</a>
                </div>
            </div>
        </div>
        <div class="work">
            <div class="row-fluid">
                <div class="span3">
                    build.xunnlv.com</div>
                <div class="span4">
                Server-side, front-end scripts
                </div>
                <div class="span2">
                    2011/10</div>
                <div class="span3">
                    <a href="http://build.xunnlv.com/" target="_blank">http://build.xunnlv.com/</a>
                </div>
            </div>
        </div>
        <div class="work">
            <div class="row-fluid">
                <div class="span3">
                    MasonryGallery</div>
                <div class="span4">
                Server-side, front-end scripts
                </div>
                <div class="span2">
                    2011/9</div>
                <div class="span3">
                    <a href="http://masonrygallery.com/" target="_blank">http://masonrygallery.com/</a>
                </div>
            </div>
        </div>
        <div class="work">
            <div class="row-fluid">
                <div class="span3">
                    Goclassing.com in Chinese</div>
                <div class="span4">
                Server-side, front-end scripts
                </div>
                <div class="span2">
                    2011/8</div>
                <div class="span3">
                    <a href="img/screenshots/gc1.png" data-fancybox-group="gc" class="fancybox" target="_blank">screenshot1</a>
                    <a href="img/screenshots/gc2.png" data-fancybox-group="gc" class="fancybox" target="_blank">screenshot2</a>
                    <a href="img/screenshots/gc3.png" data-fancybox-group="gc" class="fancybox" target="_blank">screenshot3</a>
                </div>
            </div>
        </div>
        <div class="work">
            <div class="row-fluid">
                <div class="span3">
                    Online course:<em>Principles of Computer OSs</em></div>
                <div class="span4">
                    Full development of web-application part
                </div>
                <div class="span2">
                    2010/11</div>
                <div class="span3">
                    <a href="img/screenshots/ocp.png" class="fancybox" target="_blank">screenshot</a>
                </div>
            </div>
        </div>
        <div class="work">
            <div class="row-fluid">
                <div class="span3">
                    Wake on Lan MCU (micro controlling unit)</div>
                <div class="span4">
                    Full development
                </div>
                <div class="span2">
                    2010/10</div>
                <div class="span3">
                    <a href="img/screenshots/lwaker.png" class="fancybox" target="_blank">screenshot</a>
                    <a href="https://github.com/layerssss/lwaker" target="_blank">source</a>
                </div>
            </div>
        </div>
        <div class="work">
            <div class="row-fluid">
                <div class="span3">
                    NETC Toolkit</div>
                <div class="span4">
                    Full development
                </div>
                <div class="span2">
                    2010/8</div>
                <div class="span3">
                    <a href="img/screenshots/netct.png" class="fancybox" target="_blank">screenshot</a>
                </div>
            </div>
        </div>
        <div class="work">
            <div class="row-fluid">
                <div class="span3">
                    Web-services at MCM training center, JNU</div>
                <div class="span4">
                    Full development
                </div>
                <div class="span2">
                    2010/6</div>
                <div class="span3">
                    <a href="img/screenshots/sms1.png" data-fancybox-group="sms" class="fancybox" target="_blank">screenshot1</a>
                    <a href="img/screenshots/sms2.png" data-fancybox-group="sms" class="fancybox" target="_blank">screenshot2</a>
                    <a href="img/screenshots/sms3.png" data-fancybox-group="sms" class="fancybox" target="_blank">screenshot3</a>
                    <a href="img/screenshots/sms4.png" data-fancybox-group="sms" class="fancybox" target="_blank">screenshot4</a>
                    <a href="img/screenshots/sms5.png" data-fancybox-group="sms" class="fancybox" target="_blank">screenshot5</a>
                    <a href="img/screenshots/sms6.png" data-fancybox-group="sms" class="fancybox" target="_blank">screenshot6</a>
                </div>
            </div>
        </div>
        <div class="work">
            <div class="row-fluid">
                <div class="span3">
                    StrikingCrafts online shop</div>
                <div class="span4">
                    Full development
                </div>
                <div class="span2">
                    2009/12</div>
                <div class="span3">
                    <a href="img/screenshots/lshop.png" class="fancybox" target="_blank">screenshot</a>
                </div>
            </div>
        </div>
        <div class="work">
            <div class="row-fluid">
                <div class="span3">
                    ASPFileEditor</div>
                <div class="span4">
                    Full development
                </div>
                <div class="span2">
                    2006/6</div>
                <div class="span3">
                    <a href="img/screenshots/afe.png" class="fancybox" target="_blank">screenshot</a>
                </div>
            </div>
        </div>
    </div>
    <div class="span12">
        <hr />
        <h3>
            Internships in University:</h3>
        <ul>
            <li>2012/4~2012/5 in <strong>Computer Center</strong> as teaching assistant of <em>Introduction to C Language Programming</em></li>
            <li>2010/8~2011/5 in <strong>Laboratory of Signal Processing</strong> as laboratory assistant</li>
            <li>2009/7~2011/5 in <strong>MCM Trainning Center</strong> as network administrator.</li>
            <li>2009/3~2011/5 in <strong>Network Maintenance Squad</strong> as student worker.</li>
        </ul>
    </div>
</div>
<!--*/

    }
});
//-->
